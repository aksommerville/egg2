...
linux_CFILES:=$(filter src/eggrt/%.c src/util/%.c $(addprefix src/opt/,$(addsuffix /%.c,$(linux_OPT_ENABLE))),$(SRCFILES))
linux_OFILES:=$(patsubst src/%.c,$(linux_MIDDIR)/%.o,$(linux_CFILES))
-include $(linux_OFILES:.o=.d)
$(linux_MIDDIR)/%.o:src/%.c;$(PRECMD) $(linux_CC) -o$@ $< $(foreach U,$(linux_OPT_ENABLE),-DUSE_$U=1)
...
ifneq (,$(strip $(linux_WAMR_SDK)))
  ...
  linux-all:$(linux_EXE)
  linux_CFILES_EXE:=$(filter src/eggrun/%.c src/util/%.c $(addprefix src/opt/,$(addsuffix /%.c,$(linux_EXE_OPT_ENABLE))),$(SRCFILES))
  ...
  $(linux_MIDDIR)/eggrun/%.o:src/eggrun/%.c;$(PRECMD) $(linux_CC) -I$(linux_WAMR_SDK)/core/iwasm/include -o$@ $< $(foreach U,$(linux_EXE_OPT_ENABLE) $(linux_OPT_ENABLE),-DUSE_$U=1)
endif

define linux_UTIL_RULES
  linux_UTIL_$1_LIB:=$(linux_OUTDIR)/lib$1.a
  linux-all:$$(linux_UTIL_$1_LIB)
  linux_UTIL_$1_OFILES:=$$(filter $(linux_MIDDIR)/util/$1/%,$(linux_OFILES))
  $$(linux_UTIL_$1_LIB):$$(linux_UTIL_$1_OFILES);$$(PRECMD) $(linux_AR) rc $$@ $$^
endef
$(foreach U,$(UTIL_UNITS),$(eval $(call linux_UTIL_RULES,$U)))
