...
int eggdev_convert_noop(struct sr_convert_context *ctx) {
...
int eggdev_sprite_from_sprtxt(struct sr_convert_context *ctx) {
  ...
  struct sr_convert_context modctx=*ctx;
  char *argv[]={0,"--ns=sprite"};
  modctx.argv=argv;
  modctx.argc=2;
...
int eggdev_png_from_png(struct sr_convert_context *ctx) {
  int w=0,h=0;
  if (image_measure(&w,&h,ctx->src,ctx->srcc)<0) {
    return sr_convert_error(ctx,"Failed to decode PNG file.");
  }
  ...
  free(pixels);
  if (err<0) return sr_convert_error(ctx,"Failed to reencode PNG file.");
  return 0;
}
