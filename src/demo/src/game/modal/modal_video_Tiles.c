...
#define MODAL ((struct modal_video_Tiles*)modal)

/* Delete.
 */
 
static void _video_Tiles_del(struct modal *modal) {
...
static void Tiles_set_count(struct modal *modal,int nc) {
  if (nc<0) nc=0;
  ...
  if (nc>MODAL->vtxc) {
    ...
    for (;addc-->0;vtx++) {
      ...
      vtx->y=rand()%FBH;
      switch (rand()%2) {
        case 0: vtx->tileid=0x20; break;
        case 1: vtx->tileid=0x21; break;
        //TODO Maybe some more variety?
      }
      vtx->xform=rand()&7;
    }
  }
...
static void Tiles_adjust_unmode(struct modal *modal,int d) {
...
static void _video_Tiles_update(struct modal *modal,double elapsed,int input,int pvinput) {
  ...
  if ((input&EGG_BTN_UP)&&!(pvinput&EGG_BTN_UP)) Tiles_set_count(modal,MODAL->vtxc+d);
  if ((input&EGG_BTN_DOWN)&&!(pvinput&EGG_BTN_DOWN)) Tiles_set_count(modal,MODAL->vtxc-d);
  
  /* LEFT or RIGHT to adjust the uniform mode.
   */
  if ((input&EGG_BTN_LEFT)&&!(pvinput&EGG_BTN_LEFT)) Tiles_adjust_unmode(modal,-1);
  if ((input&EGG_BTN_RIGHT)&&!(pvinput&EGG_BTN_RIGHT)) Tiles_adjust_unmode(modal,1);
}

/* Render.
 */
 
static void _video_Tiles_render(struct modal *modal) {
  graf_flush(&g.graf);
...
static int _video_Tiles_init(struct modal *modal) {
  modal->del=_video_Tiles_del;
  modal->update=_video_Tiles_update;
  modal->render=_video_Tiles_render;
  
  MODAL->mode_texid=egg_texture_new();
  Tiles_adjust_unmode(modal,0); // To generate the initial message texture.
  MODAL->msg_texid=egg_texture_new();
  Tiles_set_count(modal,100);
...
struct modal *modal_new_video_Tiles() {
  struct modal *modal=modal_new(sizeof(struct modal_video_Tiles));
  if (!modal) return 0;
  if (
    (_video_Tiles_init(modal)<0)||
    (modal_push(modal)<0)
  ) {
    modal_del(modal);
    return 0;
